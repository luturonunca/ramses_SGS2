...
subroutine collect_acczone_avg_np(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  !real(dp),dimension(1:nvector)::r2sink     
  ...
  do ind=1,twotondim
     do j=1,np
        if(ok(j,ind))then
           ...
           ! --------------------------
           !vgas_arr(j,1)=vgas_arr(j,1)+d*vv(1)*vol(j,ind)
           !vgas_arr(j,2)=vgas_arr(j,2)+d*vv(2)*vol(j,ind)
           !vgas_arr(j,3)=vgas_arr(j,3)+d*vv(3)*vol(j,ind)
           !c2_gas(j)=c2_gas(j)+d*c2*vol(j,ind)
           ! Get sink index
           ...
           !wmom(isink,1)=wmom(isink,1)+weight*d*vgas_arr(j,1)
  ...
  !         !wmom(isink,1:ndim)=wmom(isink,1:ndim)+weight*d*vv(1:ndim)
  !         wmom(isink,1)=wmom(isink,1)+weight*d*vgas_arr(j,1)
  !         wmom(isink,2)=wmom(isink,2)+weight*d*vgas_arr(j,2)
  !         wmom(isink,3)=wmom(isink,3)+weight*d*vgas_arr(j,3)
  !         wdmg(isink)=wdmg(isink)+weight*dmgas(j)
  !         w_c2  (isink)=w_c2  (isink)+weight*c2_gas(j)
  !
  !end do
...
subroutine compute_accretion_rate(write_sinks)
  ...
  do isink=1,nsink
     ...
     density=0.d0; volume=0.d0; velocity=0.d0; ethermal=0d0; c2mean=0d0
