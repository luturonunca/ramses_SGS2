module pm_commons
  ...
  integer(1),parameter :: FAM_DM=1, FAM_STAR=2, FAM_CLOUD=3, FAM_DEBRIS=4, FAM_OTHER=5, FAM_BNS=6, FAM_UNDEF=127
  ...
  character(len=13), dimension(-NFAMILIES:NFAMILIES), parameter :: particle_family_keys = (/ &
       '   bns_tracer', ' other_tracer', 'debris_tracer', ' cloud_tracer', '  star_tracer', ' other_tracer', &
       '   gas_tracer', &
       '           DM', '         star', '        cloud', '       debris', '        other', '          bns'/)
...
contains
  ...
  elemental logical pure function is_bns(typep)
    type(part_t), intent(in) :: typep
    is_bns = typep%family == FAM_BNS
  end function is_bns
